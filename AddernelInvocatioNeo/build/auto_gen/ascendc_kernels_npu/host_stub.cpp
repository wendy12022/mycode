...
struct ascend_kernels {
    ...
    uint8_t aiv_buf[144936];
} __ascend_kernel_ascend910b4_ascendc_kernels_npu __attribute__ ((section (".ascend.kernel.ascend910b4.ascendc_kernels_npu"))) = {1,1,1,144936,144936,{0}};
...
void __register_kernels(void)
{
    ...
    ret = RegisterAscendBinary(
        (const char *)__ascend_kernel_ascend910b4_ascendc_kernels_npu.aiv_buf,
        __ascend_kernel_ascend910b4_ascendc_kernels_npu.aiv_file_len,
        1,
        &g_kernel_handle_aiv);
    if (ret != 0) {
...
uint32_t launch_and_profiling_add_custom(uint64_t func_key, uint32_t blockDim, void* stream, void **args, uint32_t size)
{
    uint64_t startTime;
...
extern "C" uint32_t aclrtlaunch_add_custom(uint32_t blockDim, void* stream, void* x, void* y, void* z)
{
    ...
    __ascendc_ret = launch_and_profiling_add_custom(0, blockDim, stream, (void **)&__ascendc_args, sizeof(__ascendc_args));
